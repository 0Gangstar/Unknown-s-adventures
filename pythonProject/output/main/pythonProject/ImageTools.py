...
def load_image(filename, scale, size=16):
    ...
    image = pygame.transform.scale(image, (int(image.get_size()[0] * scale * size / Player_stats.tile_size), int(image.get_size()[1] * scale * size / Player_stats.tile_size))).convert_alpha()
    return image
